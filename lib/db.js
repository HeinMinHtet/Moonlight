...
export async function insertRaidSchedule(schedule, session = {}) {
  ...
  const record = {
    ...
    date: String(schedule.date || "").trim() || now.slice(0, 10),
    timeEst: String(schedule.timeEst || "").trim(),
    raid: String(schedule.raid || "").trim() || "Venomous Abyss",
    difficulty,
    loot,
    maxBuyers,
    buyers: sanitizedBuyers,
    lead: String(schedule.lead || "").trim(),

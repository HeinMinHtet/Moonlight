      ...
      if (settleWithdrawals) {
        ...
        for (const w of activeWithdrawalsRes.rows) {
          ...
          if (amt <= remainingOffset) {
            ...
            remainingOffset = 0;
            await client.query(
              "UPDATE supplier_withdrawals SET amount = $1, settled = true, settled_at = $2, settlement_batch_id = $3, note = $4, updated_at = $2 WHERE id = $5",
              [offsetAmt, paidAt, paymentBatchId, `${w.note || ""} (Partial offset applied)`.trim(), w.id]
            );
            ...
            await client.query(`
              INSERT INTO supplier_withdrawals (
                id, date, char_name, guild, amount, note, settled, settled_at,
                settlement_batch_id, created_by_discord_id, created_by_name, created_at
              ) VALUES ($1, $2, $3, $4, $5, $6, false, null, null, $7, $8, $9)
            `, [
    ...
    try {
      ...
      for (const [key, runAmount] of boosterTotals.entries()) {
        const adjsRes = await client.query(
          "SELECT * FROM booster_adjustments WHERE (discord_id = $1 OR booster_name = $1) AND (settled IS NULL OR settled = false) ORDER BY date ASC, created_at ASC",
          [key]
        );
  ...
  if (isPostgres()) {
    ...
    try {
      const [supplierRes, boosterRes, expenseRes, adjRes] = await Promise.all([
        ...
        client.query("SELECT * FROM external_expenses"),
        client.query("SELECT * FROM booster_adjustments WHERE settled = true")
      ]);
      const db = {

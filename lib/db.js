  ...
  if (isPostgres()) {
    ...
    try {
      const [servicesRes, boosterRes, armorRes] = await Promise.all([
        client.query("SELECT type, price, active FROM supplier_services ORDER BY id ASC"),
        client.query("SELECT level, price, active FROM booster_prices ORDER BY id ASC"),
      ...
      return {
        ...
        boosterPrices: permissions.priceSettings
          ? boosterPrices
          : boosterPrices.filter((row) => row.active !== false).map(({ level }) => ({ level, price: 0, active: true })),
        armorTypes
      };
    } finally {
      client.release();
...
export async function getSupplierServicesList() {
  if (isPostgres()) {
    const client = await getPool().connect();
    try {
      const res = await client.query("SELECT type, price, active FROM supplier_services ORDER BY id ASC");
      return res.rows.map((row) => ({ type: row.type, price: Number(row.price), active: Boolean(row.active) }));
    } finally {
      client.release();
    }
  } else {
...
export async function getBoosterPricesList() {
  if (isPostgres()) {
    const client = await getPool().connect();
    try {
      const res = await client.query("SELECT level, price, active FROM booster_prices ORDER BY id ASC");
      return res.rows.map((row) => ({ level: row.level, price: Number(row.price), active: Boolean(row.active) }));
    } finally {
      client.release();
    }
  } else {
  ...
  if (isPostgres()) {
    ...
    try {
      ...
      for (const row of cleanedRows) {
        await client.query(
          "INSERT INTO supplier_services (type, price, active) VALUES ($1, $2, $3)",
          [row.type, row.price, row.active]
        );
      }
      await client.query("COMMIT");
      return cleanedRows;
...
export async function updateBoosterPrices(cleanedRows) {
  if (isPostgres()) {
    ...
    try {
      ...
      for (const row of cleanedRows) {
        await client.query(
          "INSERT INTO booster_prices (level, price, active) VALUES ($1, $2, $3)",
          [row.level, row.price, row.active]
        );
      }
      await client.query("COMMIT");
      return cleanedRows;

...
export function BuyerPurchaseTypesPanel({
  title = "Buyer purchase types",
  rows = [],
  notes = [],
  disabled = false,
  onAdd,
  ...
  const duplicateNames = useMemo(() => findDuplicateNames(rows), [rows]);
      ...
      <form className="price-panel flex flex-col overflow-hidden" onSubmit={onSubmit}>
        <div className="section-head flex flex-wrap items-center justify-between gap-3 border-b border-border/70 p-4">
          <div>
            ...
            <p className="panel-note text-xs text-muted-foreground">Purchase types available when adding buyers.</p>
          </div>
        ...
        <div className="rate-table-wrapper w-full overflow-x-auto">
          <div className="min-w-[440px]">
            ...
            >
              <span className="text-center" title="Default selection for new buyers">Def</span>
            ...
            <div className="rate-table-body max-h-[460px] divide-y divide-border/40 overflow-y-auto">
              {displayedRows.length === 0 ? (
                <div className="py-10 text-center text-xs text-muted-foreground">
                  {activeTab === "active"
                    ? "No active types. Restore an archived type or add a new one."
                    : "No archived types."}
                </div>
              ) : (
                displayedRows.map(({ row, index }) => {
                  ...
                  // History logic: find if any buyer item in any note has this purchaseType
                  const historyCount = notes.filter((n) => 
                    (n.items || []).some(item => String(item.purchaseType || "").trim().toLocaleLowerCase() === normalizedName)
                  ).length;
                    ...
                    >
                      ...
                      <div className="flex justify-center">
                        <Button
                          ...
                          title={isDefault ? "Default selection for new buyers" : "Set as default type"}
                          aria-label={isDefault ? `Unmark ${row.name || "type"} as default` : `Set ${row.name || "type"} as default`}
                        >
                          <Star
                            className={cn(
                      ...
                      <div className="flex justify-center">
                        {historyCount > 0 ? (
                          <Badge
                            ...
                            title={`Used by ${historyCount} raid note${historyCount === 1 ? "" : "s"}.`}
                          >
                            {historyCount} used
                          </Badge>
                        ) : (
                          <Badge variant="neutral" className="text-[10px] px-1.5 py-0.5 opacity-60 whitespace-nowrap" title="Not used by any buyers.">
                            0 used
                          </Badge>
                        )}
                      </div>
                      ...
                      <div className="flex items-center justify-end gap-1.5">
                        <Button
                          ...
                          disabled={disabled}
                          title={isArchived ? "Restore type" : "Archive type"}
                        >
                        ...
                        {historyCount === 0 ? (
                          <Button
                            ...
                            disabled={disabled}
                            title={`Delete ${row.name || "type"}`}
                            aria-label={`Delete ${row.name || "type"}`}
                          >

...
export function RaidNoteTitlesPanel({
  title = "Raid note titles",
  rows = [],
  notes = [],
  disabled = false,
  onAdd,
      ...
      <form className="price-panel flex flex-col overflow-hidden" onSubmit={onSubmit}>
        <div className="section-head flex flex-wrap items-center justify-between gap-3 border-b border-border/70 p-4">
          <div>
            ...
            <p className="panel-note text-xs text-muted-foreground">Titles available when creating raid notes.</p>
          </div>
        ...
        <div className="rate-table-wrapper w-full overflow-x-auto">
          <div className="min-w-[440px]">
            ...
            >
              <span className="text-center" title="Default selection for new notes">Def</span>
            ...
            <div className="rate-table-body max-h-[460px] divide-y divide-border/40 overflow-y-auto">
              {displayedRows.length === 0 ? (
                <div className="py-10 text-center text-xs text-muted-foreground">
                  {activeTab === "active"
                    ? "No active titles. Restore an archived title or add a new one."
                    : "No archived titles."}
                </div>
              ) : (
                displayedRows.map(({ row, index }) => {
                  ...
                  const historyCount = notes.filter((n) =>
                    String(n.title || "").trim().toLocaleLowerCase() === normalizedName
                  ).length;
                    ...
                    >
                      ...
                      <div className="flex justify-center">
                        <Button
                          ...
                          title={isDefault ? "Default selection for new notes" : "Set as default title"}
                          aria-label={isDefault ? `Unmark ${row.name || "title"} as default` : `Set ${row.name || "title"} as default`}
                        >
                          <Star
                            className={cn(
                      ...
                      <div className="flex justify-center">
                        {historyCount > 0 ? (
                          ...
                          <Badge variant="neutral" className="text-[10px] px-1.5 py-0.5 opacity-60 whitespace-nowrap" title="Not used by any notes.">
                            0 used
                          </Badge>
                        )}
                      </div>
                      ...
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        {historyCount === 0 ? (
                          <Button
                            ...
                            disabled={disabled}
                            title={`Delete ${row.name || "title"}`}
                            aria-label={`Delete ${row.name || "title"}`}
                          >
